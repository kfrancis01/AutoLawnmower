...
//   and set at compile time and never modified during execution.//#include <SoftwareSerial.h>  //needed only if using UNO(one hardware serial port) board.
...
if (Status == 1){ // Bluetooth activation (Status = 1 -> Green; Status = 0 -> Red)
  if(chops == 0){ // If first loop of a new row
    ...
    timeDelay = 5e6; // 5 seconds in microseconds
